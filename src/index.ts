 ...
 *
 * Host: swapi.dev is gone — the name no longer resolves, so every call came
 * back 530 (what a Worker fetch to an NXDOMAIN host looks like) and read as a
 * flaky upstream rather than a dead one. swapi.py4e.com is the maintained
 * mirror of the original Django app: identical paths, identical response
 * shapes, and it still supports `?search=`. If it ever goes the same way,
 * swapi.info is a static mirror — but it returns bare arrays and ignores
 * `search`, so switching to it means filtering locally, not just a URL swap.
 *
 * Tools:
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'SWAPI');
}

const BASE = 'https://swapi.py4e.com/api';

/** 5xx/429 mean the mirror is unwell, not that the caller asked wrong. */
function upstream(label: string, status: number): Error {
  const prefix = status >= 500 || status === 429 ? 'upstream_down: ' : '';
  return new Error(`${prefix}SWAPI ${label} error: ${status}`);
}
...
async function searchPeople(query: string) {
  ...
  const res = await pwFetch(`${BASE}/people/?${params}`);
  if (!res.ok) throw upstream('people search', res.status);
...
async function getPlanet(id: number) {
  const res = await pwFetch(`${BASE}/planets/${id}/`);
  if (res.status === 404) throw new Error(`Planet not found: ID ${id}`);
  if (!res.ok) throw upstream('planet', res.status);
...
async function getStarship(id: number) {
  const res = await pwFetch(`${BASE}/starships/${id}/`);
  if (res.status === 404) throw new Error(`Starship not found: ID ${id}`);
  if (!res.ok) throw upstream('starship', res.status);
...
async function getFilm(id: number) {
  const res = await pwFetch(`${BASE}/films/${id}/`);
  if (res.status === 404) throw new Error(`Film not found: ID ${id}`);
  if (!res.ok) throw upstream('film', res.status);
